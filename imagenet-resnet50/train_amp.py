...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--lr-middle-epochs", type=int, default=100)
...
def test_loop(
    ...
    with torch.no_grad():
        for batch, (inputs, targets) in enumerate(test_dataloader, start=batch):
            ...
            # Create one-hot encoded targets for test BCE loss calculation
...
def main(args, timer):
    ...
    train_transform = v2.Compose(
        [
            v2.PILToTensor(),
            # v2.RandomResizedCrop(176, antialias=True), # Composer recommends training at lower res
    ...
    test_transform = v2.Compose(
        [
            v2.PILToTensor(),
            # FixRes ratio 
            # v2.Resize(232, antialias=True), # Composer specification
    ...
    if args.optim  == 'sgd':
        ...
        raise Exception("Arg 'optim' must be either 'sgd', 'adamw', or 'lamb'.")

    # Number of steps for each learning rate schedule phase based on lr_stepevery
    if args.lr_stepevery == 'batch':
        warmup_steps = args.lr_warmup_epochs * len(train_dataloader)
        middle_steps = args.lr_middle_epochs * len(train_dataloader)
        decay_steps = args.epochs * len(train_dataloader) - warmup_steps - middle_steps
    elif args.lr_stepevery == 'epoch':
        warmup_steps = args.lr_warmup_epochs
        middle_steps = args.lr_middle_epochs
        decay_steps = args.epochs - warmup_steps - middle_steps

    # Helpful learning rate scheduler primitives
    def get_linear_scheduler(start, finish, steps):
        linear_lambda = lambda step: step * (finish - start) / steps + start
        return torch.optim.lr_scheduler.LambdaLR(optimizer, linear_lambda)
    def get_cosine_scheduler(start, finish, steps):
    ...
    def get_exp_scheduler(start, finish, steps):
    ...
    warmup_scheduler = get_exp_scheduler(0.1, 1.0, warmup_steps)
    middle_scheduler = get_linear_scheduler(1.0, 1.0, middle_steps)
    decay_scheduler = get_cosine_scheduler(1.0, 0.001, decay_steps)

    # Assemble each-phase schedulers into overall sequence
    lr_sched_milestones = [warmup_steps - 1, warmup_steps + middle_steps - 1]
    lr_scheduler = torch.optim.lr_scheduler.SequentialLR(
        optimizer, schedulers=[warmup_scheduler, middle_scheduler, decay_scheduler], milestones=lr_sched_milestones
    )

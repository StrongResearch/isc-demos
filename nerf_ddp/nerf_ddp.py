...
# Training
n_iters = 10000
batch_size = 2**1500          # Number of rays per gradient step (power of 2)
one_image_per_step = True   # One image per gradient step (disables batching)
# TODO: optimze its size to maximize the speed
chunksize = 2**150          # Modify as needed to fit in GPU memory
...
# Gather as torch tensors
  ...
  for i in trange(n_iters):
    ...
    if one_image_per_step:
        ...
        partitions = list(range(0, rays_o.shape[0], int(rays_o.shape[0]/(world_size))))
...
#     os.environ["MASTER_ADDR"] = "10.10.10.69"

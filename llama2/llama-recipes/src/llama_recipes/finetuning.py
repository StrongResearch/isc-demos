...
def main(**kwargs):
    ...
    if train_config.enable_fsdp and train_config.low_cpu_fsdp:
        ...
        if rank == 0:
            ...
            timer.report(f"finished loading LlamaForCausalLM.from_pretrained")
        else:
    ...
    timer.report("model and tokenizer")
    ...
    if train_config.enable_fsdp and fsdp_config.pure_bf16:
        ...
        timer.report("model to bfloat16")

    ## -- ADJUST TO LOAD PRE-SAVED MODULES IF RESUMING -- ##
    ...
    if train_config.enable_fsdp:
        ...
        timer.report("FSDP wrapped")
        if fsdp_config.fsdp_activation_checkpointing:
            apply_fsdp_checkpointing(model)
            timer.report("FSDP checkpointing")
    ...
    if train_config.run_validation:
        ...
        ## -- INSERT INTERRUPTIBLE DISTRIBUTED SAMPLER -- ##
        eval_sampler = InterruptableDistributedSampler(dataset_val)
        val_dl_kwargs["sampler"] = eval_sampler
    ...
    for epoch in range(start_epoch, train_config.num_epochs):
        ...
        with train_dataloader.sampler.in_epoch(epoch):
            ...
            with eval_dataloader.sampler.in_epoch(epoch):

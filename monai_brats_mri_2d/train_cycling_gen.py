...
timer.report('importing torch functions')

from torchvision.transforms import Compose, RandomHorizontalFlip, RandomVerticalFlip
from torch.utils.data import DataLoader

timer.report('importing torchvision functions')
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--nepochs", default=10, type=int, help="number of training epochs")
    parser.add_argument("--lr", default=5e-5, type=float, help="initial learning rate")
    parser.add_argument("--batch-size", default=16, type=int, help="local batch size")
    parser.add_argument("--eval-batch-size", default=8, type=int, help="local batch size")
    ...
    parser.add_argument("--warmup-epochs", default=100, type=int, help="autoencoder warmup epochs")
    return parser

timer.report('defined argparser')

def main(args, timer):

    ## CAN THIS BE REPLACED WITH STANDARD INIT FROM FASHION MNIST?
    ...
    train_ds = utils.Tensor3dDataset(args.data_path, "train", transform=transform)
    val_ds = utils.Tensor3dDataset(args.data_path, "val", transform=None)
    ...
    # Original trainer had batch size = 26. Using 12 nodes x 6 GPUs x batch size 2 = eff batch size = 144
    train_loader = DataLoader(train_ds, batch_size=args.batch_size, sampler=train_sampler, num_workers=1)
    val_loader = DataLoader(val_ds, batch_size=args.eval_batch_size, sampler=val_sampler, num_workers=1)
    ...
    # Optimizers
    optimizer_g = torch.optim.Adam(generator_without_ddp.parameters(), lr=args.lr)
    optimizer_d = torch.optim.Adam(discriminator_without_ddp.parameters(), lr=args.lr)
    ...
    # ## TEMP CHECKPOINT INSERT
    # print("RE-INSERTING FROM BEST MODEL LAST RUN")
    # ckp_path = "/mnt/Client/Adamstn3rh22tykvgyhdkclook3rnk7q/adaadam4qalumfvjdstjpx7zyvlebh2u/isc-demos/monai_brats_mri_2d/models_backup/generator.pt"
    # ckp = torch.load(ckp_path, map_location="cpu")
    # generator_without_ddp.load_state_dict(ckp["best_model"])
    # timer.report('RE-INSERTION COMPLETE')

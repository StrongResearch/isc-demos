...
#         if "backbone.body" not in '.'.join(path):
#             for p in model.parameters():
#                 p = torch.nn.Parameter((2 * torch.rand(p.shape, dtype=p.dtype) - 1) * scale)
#         # print(f"found leaf at {path} {name}")
#     else:
#         for n,c in model.named_children():
#             init_params(c, scale, path + [name], n)
...
def main(args, timer):
    ...
    if args.model.endswith("resnet50_fpn"):
        backbone = resnet_fpn_backbone(backbone_name="resnet50", weights="ResNet50_Weights.IMAGENET1K_V1", trainable_layers=3)
    elif args.model.endswith("resnet101_fpn"):
        backbone = resnet_fpn_backbone(backbone_name='resnet101', weights='ResNet101_Weights.IMAGENET1K_V1', trainable_layers=3)
        # if args.model.startswith("retinanet"):
        #     # backbone = resnet_fpn_backbone(backbone_name='resnet101', weights='ResNet101_Weights.DEFAULT', trainable_layers=0)
        #     backbone = resnet_fpn_backbone(backbone_name='resnet101', weights='ResNet101_Weights.IMAGENET1K_V1', trainable_layers=3)
        # else:
        #     backbone = resnet_fpn_backbone(backbone_name='resnet101', weights='ResNet101_Weights.IMAGENET1K_V1', trainable_layers=3)
    else:
    ...
    # Shrink non-backbone body params
    # init_params(model)
    ...
    warmup_lr_scheduler = torch.optim.lr_scheduler.LinearLR(
        optimizer, start_factor=0.1, total_iters=steps_per_epoch
        # optimizer, start_factor=1/steps_per_epoch, total_iters=steps_per_epoch
    )
    ...
    #     long_lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.epochs)
    # else:
    #     raise RuntimeError(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print_rank0(f"\nEPOCH :: {epoch}\n")
        # Create new coco_evaluator each epoch
        coco_evaluator = CocoEvaluator(coco, iou_types)
